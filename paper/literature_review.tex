...
从已有研究看，文本风格迁移的评价难点主要来自内容与风格之间的耦合。一方面，模型需要改变表达方式，使输出符合目标风格；另一方面，模型又不能为了追求风格明显而改写事实内容。Shen 等人较早将非平行语料条件下的文本风格迁移建模为内容表示与风格表示的解耦问题，通过对抗训练约束隐变量表示尽量去除风格信息，再利用目标风格条件生成句子\cite{shen2017styletransfer}。该思路说明，风格迁移并不是简单的同义句改写，而是需要在内容保持和风格控制之间取得平衡。对于本项目而言，“老板、同事、朋友、恋人、母亲”等目标对象并不只是几个表层标签，而是对应不同的礼貌程度、亲疏距离、称呼方式和信息组织策略，因此更需要关注内容约束与风格约束的同时满足。

Li 等人提出的 Delete, Retrieve, Generate 方法进一步强调了风格词和内容词之间的区分：模型先删除源句中的风格相关片段，再从目标风格语料中检索相关表达，最后生成目标风格句子\cite{li2018delete}。这一方法对本项目有两点启发。第一，角色风格往往体现在局部词汇和短语中，例如“您”“麻烦”“宝贝”“妈”等称呼与语气词具有明显对象指向性；第二，风格迁移不能只依赖局部替换，因为不同对象下的信息重点也会变化。例如对老板表达时可能突出进度和责任，对母亲表达时可能突出安全和安抚。因此，本项目在评价时不能只检查是否出现角色关键词，还应判断整句的信息组织是否符合目标对象。

在评价方法方面，Mir 等人系统分析了文本风格迁移中的自动评价与人工评价问题，指出 BLEU、ROUGE、风格分类准确率和语言模型困惑度等指标只能覆盖部分质量维度，难以单独反映迁移结果的整体可用性\cite{mir2019evaluating}。例如，较高的 BLEU 可能意味着模型保留了大量原句内容，但也可能说明模型没有完成有效迁移；较高的风格分类准确率可能来自几个强风格词，却不能保证语义没有漂移。该观点与本项目高度相关：关系角色风格迁移的目标不是生成“看起来像某类角色”的句子，而是让说话者在面对不同对象时表达得合适、自然且不改变原意。因此，本项目应采用自动规则检查与人工评价结合的方式，避免单一指标造成误判。
