...
模型训练脚本位于 \path{finetune/train_lora.py}。训练完成后适配器保存至 \path{finetune/output/final/}，后端推理服务加载 mT0-large 基座模型和该 LoRA 适配器。
...
\begin{figure}[H]
\centering
\fbox{\parbox[c][4cm][c]{10cm}{\centering 待补充：正式微调后放入 \texttt{paper/figures/train\_eval\_loss\_curve.png}}}
...
模型输出中的错误可按表~\ref{tab:error_analysis} 进行归类。后续可从系统测试或测试集推理结果中选取真实样例，补充“原句、目标角色、错误输出、原因分析、改进方向”。
